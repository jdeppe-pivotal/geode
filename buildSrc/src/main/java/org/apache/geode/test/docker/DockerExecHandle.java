 ...
 * </ul>
 */
public class DockerExecHandle implements ExecHandle, ProcessSettings {

  private static final Logger LOGGER = Logging.getLogger(DockerExecHandle.class);
...
//  private final ProcessLauncher processLauncher;
   ...
   */
  private DockerExecHandleRunner execHandleRunner;
...
//    processLauncher = NativeServices.getInstance().get(ProcessLauncher.class);
  ...
  public ExecHandle start() {
    LOGGER.info("Starting process '{}'. Working directory: {} Command: {}",
        displayName, directory, command + ' ' + String.join(" ", arguments));
...
//      invokeIfNotNull(extension.getAfterContainerStart(), containerId, client);
      ...
      Process proc = new DockerProcess(client, containerId);

      return proc;
    } catch (Exception e) {
      e.printStackTrace();
      throw new GradleException("Unable to launch container", e);
    }
  }
